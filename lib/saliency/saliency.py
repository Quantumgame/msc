...
def intensityConspicuity(image):
	...
	fs = features(image, channel = intensity)
...
def gaborConspicuity(image, steps):
	...
	for step in range(steps):
		...
		gaborFeatures = features(intensity(image), channel = gaborFilter)
		summedFeatures = sumNormalizedFeatures(gaborFeatures)
		#gaborConspicuity += N(summedFeatures)
                #felipe
		gaborConspicuity +=\
                N(summedFeatures).astype(gaborConspicuity.dtype)
...
def sumNormalizedFeatures(features, levels=9, startSize=(640,480)):
    ...
    """
    commonWidth = startSize[0] / 2**(int(levels/2) - 1)
    commonHeight = startSize[1] / 2**(int(levels/2) - 1)
    #felipe
...
def N(image):
	...
	M = 8.	# an arbitrary global maximum to which the image is scaled.
			...
			# on this constant)
	image = cv2.convertScaleAbs(image, alpha=M/(image.max()+0.0001), beta=0.)
	w,h = image.shape
...
def makeNormalizedColorChannels(image, thresholdRatio=10.):
    ...
    R = (r - (g + b) / 2).astype(numpy.uint8)
    G = (g - (r + b) / 2).astype(numpy.uint8)
    B = (b - (g + r) / 2).astype(numpy.uint8)
    Y = ((r + g) / 2 - cv2.absdiff(r,g) / 2 - b).astype(numpy.uint8)
    ...
    #ind = Y < 0
    #Y[ind] = 0
    cv2.threshold(src=R, dst=R, thresh=0, maxval=0, type=cv2.THRESH_TOZERO)
...
if __name__ == "__main__":
    ...
    if args.fileList is None and args.inputFile is None:
            ...
            for filename in filenames:
                    ...
                    #felipe
                    saliency = cv2.resize(saliency, dsize=(640,480))
                    saliency = saliency.astype(numpy.uint8)
                    cv2.imshow('static sal', saliency)
                    cv2.waitKey(2000)
