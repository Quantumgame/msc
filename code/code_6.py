...
Observations :
    ...
    Difference from code_5:

    Inhibition of return implemented. When exploring, the robot asks 'what is it?' and, 
    upon receiving the answer, updates its IOR table, which consists of three fields for 
    every robot pose: the pose, the 'yes' field and the 'no' field. The 'yes' field contains 
    the category of the object visualized by the corresponding pose. This is filled when 
    exploring or when making a correct guess when searching. The 'no' field contains the
    category to which the visualized object in the corresponding pose doesn't belong. This 
    is filled when an incorrect guess is made when searching.
