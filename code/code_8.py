...
Observations :
    ...
    Differences from code_7:

        DONE: Major overhaul. When in exploration mode, nothing is changed. When in 
        search mode, the bounding box proposal Edgebox is employed instead of the 
        traditional visual attention based. 

        DONE: Feature extraction is much faster. Googlenet is initialized only once when the program 
        starts up. In other versions, it was initialized every time an image was to be described
